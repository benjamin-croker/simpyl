...
def set_run(run_result_id, description):
    ...
    logging.basicConfig(level=logging.INFO, format='%(asctime)s %(message)s',
                        datefmt='%Y-%m-%d %H:%M:%S',
                        filename=os.path.join('runs', run_result_id,
...
def run(sl, run_init):
    ...
    for run_order, proc_init in enumerate(run_init['proc_inits']):
        ...
        sl.simpyl_log("Procedure {} called with arguments {}".format(run_result['id'], kwargs))
        ...
        db.register_proc_result(proc_result, run_result['id'])
